...
class AIProcessor {
    ...
    async init(): Promise<void> {
        ...
        try {
            ...
            console.error(
                "或者在项目中安装: npm install @anthropic-ai/claude-code",
            );
...
`;
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (
                        ...
                        files.push(
                            ...(await this.getFilesRecursively(fullPath)),
                        );
                    }
    ...
    async getTargetFiles(): Promise<string[]> {
        try {
            ...
            throw new Error(
                `无法访问目标路径 ${this.targetPath}: ${String(error)}`,
            );
        }
    }
    ...
    async processFile(
        ...
        while (true) {
            ...
            try {
                ...
                if (
                    processedError.stack !== undefined
...
function parseArgs(args: string[]): ParsedArgs {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "-h" || arg === "--help") {
            ...
            const threads = args[i + 1];
            result.threads = threads ? parseInt(threads, 10) : 1;
            i++;
        }
    }
...
用法:
  npm run ai -p <路径> -c <配置文件> [选项]

必选参数:
  -p, --path     要处理的文件或目录路径
  -c, --config   包含 AI 提示内容的文件路径

可选参数:
  -f, --filter   用于过滤文件的正则表达式
  -t, --threads  并发处理线程数 (默认: 1)
  -h, --help     显示帮助信息

依赖:
  - 需要安装 Claude Code CLI:
    - 全局安装: npm install -g @anthropic-ai/claude-code
    - 或项目安装: npm install @anthropic-ai/claude-code
...
示例:
  npm run ai -p ./src -c ./SPEC.md
  npm run ai -p ./src -c ./prompt.txt -f "\\.ts$" -t 4
  npm run ai -p ./src/index.ts -c ./prompt.txt
`);
        return;
    }
