 ...
 *
 * @example
 * type Keys = EnumKeys<typeof Enum>;
 */
 ...
 *
 * @example
 * type Values = EnumValues<typeof Enum>;
 */
export type EnumValues<T extends object> = T[keyof T];
 ...
 * const result = ['A', 'B'];
 */
export function getEnumKeys<T extends object>(tsEnum: T): (keyof T)[] {
 ...
 * const result = [1, 2];
 */
export function getEnumValues<T extends object>(tsEnum: T): number[] {
    const result: number[] = [];
    for (const key in tsEnum) {
        if (Object.hasOwn(tsEnum, key)) {
 ...
 * @param value 成员值
 */
export function isEnumMember<T extends object>(
    tsEnum: T,
    key: string,
    value: number,
) {
 ...
 * @param key 成员键
 */
export function isEnumKey<T extends object>(tsEnum: T, key: string) {
    for (const k in tsEnum) {
        if (!Number.isNaN(Number(k))) continue;
        if (Object.hasOwn(tsEnum, k)) {
 ...
 * @param value 成员值
 */
export function isEnumValue<T extends object>(tsEnum: T, value: number) {
    for (const key in tsEnum) {
        if (Object.hasOwn(tsEnum, key)) {
