...
class FlatMap<In, Out> implements IPipe<In, Out> {
  ...
  constructor(
    private callbackFn: (
      value: In,
      index: number,
    ) => Iterator<Out, unknown, undefined> | Iterable<Out, unknown, undefined>,
  ) {}
...
class Reduce<T, U> implements IPipe<T, T | U | undefined, T | U | undefined> {
  ...
  transform(input: T, next: Next<never>): void {
    if (!this.hasInitialValue) {
      ...
      this.accumulator = this.callbackFn(this.accumulator!, input, this.index);
    }
    this.index++;
  }
...
class ToArrayWithCount<T>
  implements
    IPipe<T, { buffer: T[]; written: number }, { buffer: T[]; written: number }>
{
  private index = 0;
