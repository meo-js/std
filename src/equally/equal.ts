...
function _equal(
  ...
  if (aCtor === bCtor) {
    ...
    if (aCtor === Error) {
      return equalError(a as Error, b as Error, ignorePrototype, comparator);
    }
  }
  ...
  loop: for (const aValue of a) {
    if (!isObject(aValue)) {
      ...
      for (const bValue of bCopy) {
        if (
          _equal(aValue, bValue, undefined, a, b, ignorePrototype, comparator)
        ) {
          if (bCopy === b) {
            bCopy = new Set(b);
          }
...
function equalMap(
  ...
  loop: for (const [aKey, aValue] of a) {
    if (!isObject(aValue)) {
      if (
        b.has(aKey)
        && _equal(aValue, b.get(aKey), aKey, a, b, ignorePrototype, comparator)
      ) {
        continue loop;
      } else {
        return false;
      ...
      for (const [bKey, bValue] of bCopy) {
        if (
          _equal(aKey, bKey, undefined, a, b, ignorePrototype, comparator)
          && _equal(aValue, bValue, aKey, a, b, ignorePrototype, comparator)
        ) {
          if (bCopy === b) {
            bCopy = new Map(b);
          }
...
function equalError(
  ...
  switch (a.constructor) {
    case SuppressedError: {
      if ((<SuppressedError>a).constructor !== b.constructor) return false;
...
function equalObject(
  ...
  for (let i = 0; i < aKeys.length; i++) {
    ...
    if (
      !_equal(value, (<checked>b)[key], key, a, b, ignorePrototype, comparator)
    ) {
      return false;
    }
  }
